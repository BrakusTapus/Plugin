...
using PlayerController = Plugin.Utilities.Structs.PlayerController;
...
public unsafe static class Player
{
    ...
    public static string HomeDataCenter => Excel.GetRow<World>(Svc.ClientState.LocalPlayer.HomeWorld.Id).DataCenter.Value.Name.ToString();
    public static string CurrentDataCenter => Excel.GetRow<World>(Svc.ClientState.LocalPlayer.CurrentWorld.Id).DataCenter.Value.Name.ToString();
    ...
    public static GameObject* GameObject => (CSGameObject*)Svc.ClientState.LocalPlayer.Address;
    ...
    public static IGameObject Target { get => Svc.Targets.Target; set => Svc.Targets.Target = value; }
    ...
    public static void Equip(uint itemID)
    {
        ...
        if (pos == null)
        {
            DuoLog.Error($"Failed to find item {Excel.GetRow<Item>(itemID)?.Name} (ID: {itemID}) in inventory");
            return;
        }
