...
public class DebugTools : Tweak<DebugToolsConfiguration>
{
    ...
    private unsafe void OnUpdate(IFramework framework)
    {
        ...
        {
            if (!Framework.Instance()->WindowInactive && GenericHelpers.IsKeyPressed([LimitedKeys.LeftControlKey, LimitedKeys.RightControlKey]) && Utils.IsClickingInGameWorld())
            {
        ...
        {
            if (Svc.KeyState.GetRawValue(VirtualKey.SPACE) != 0 || GenericHelpers.IsKeyPressed(LimitedKeys.Space))
            {
            ...
            if (Svc.KeyState.GetRawValue(VirtualKey.LSHIFT) != 0 || GenericHelpers.IsKeyPressed(LimitedKeys.LeftShiftKey))
            {
                Svc.KeyState.SetRawValue(VirtualKey.LSHIFT, 0);
                Player.Position = (Player.Object.Position.X, Player.Object.Position.Y - Config.NoClipSpeed, Player.Object.Position.Z).ToVector3();
            }
            if (Svc.KeyState.GetRawValue(VirtualKey.W) != 0 || GenericHelpers.IsKeyPressed(LimitedKeys.W))
            {
            ...
            if (Svc.KeyState.GetRawValue(VirtualKey.S) != 0 || GenericHelpers.IsKeyPressed(LimitedKeys.S))
            {
                var newPoint = Utils.RotatePoint(Player.Object.Position.X, Player.Object.Position.Z, MathF.PI - Player.CameraEx->DirH, Player.Object.Position + new Vector3(0, 0, -Config.NoClipSpeed));
                Svc.KeyState.SetRawValue(VirtualKey.S, 0);
                Player.Position = newPoint;
            }
            if (Svc.KeyState.GetRawValue(VirtualKey.A) != 0 || GenericHelpers.IsKeyPressed(LimitedKeys.A))
            {
            ...
            if (Svc.KeyState.GetRawValue(VirtualKey.D) != 0 || GenericHelpers.IsKeyPressed(LimitedKeys.D))
            {
                var newPoint = Utils.RotatePoint(Player.Object.Position.X, Player.Object.Position.Z, MathF.PI - Player.CameraEx->DirH, Player.Object.Position + new Vector3(-Config.NoClipSpeed, 0, 0));
